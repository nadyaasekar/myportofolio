...
## REFLEKSI TUGAS INDIVIDUAL 3
1. saya menggunakan **ModelForm** dibanding membuat form HTML manual karena field, tipe input, dan validasinya sudah otomatis diturunkan langsung dari model (seperti `Experience` dan `Education`), jadi saya tidak perlu menulis ulang aturan validasi atau memetakan tiap field satu-satu di view. proses simpan dan update data pun jadi lebih ringkas karena `ModelForm` sudah terhubung langsung ke method `.save()` pada model, sehingga kalau ke depannya ada field baru di model, cukup ubah `fields` di `Meta` tanpa perlu bongkar HTML maupun logic view. **{% csrf_token %}** wajib ditambahkan karena Django menolak semua request POST yang tidak menyertakan token tersebut; token ini melindungi dari serangan **CSRF**, di mana situs lain bisa memanfaatkan sesi login saya untuk diam-diam mengirim request berbahaya (misalnya menghapus data lewat `delete_education`) tanpa sepengetahuan saya.
2. **JSON** lebih disukai dibanding **XML** karena strukturnya jauh lebih ringkas, tidak ada tag pembuka-penutup berulang seperti XML sehingga ukuran data yang dikirim lebih kecil dan lebih cepat diproses jaringan. struktur key-value pada JSON juga lebih dekat dengan struktur data yang umum dipakai di hampir semua bahasa pemrograman, sehingga proses parsing dan pemetaan datanya jadi lebih intuitif dan ringan secara komputasi.
3. alur yang terjadi dimulai saat fungsi view menerima request, lalu mengambil data dari database lewat Django ORM. data hasil query ini masih berbentuk objek Python yang tidak bisa langsung dikirim sebagai teks lewat HTTP response, karena itu perlu melewati proses **serialization** menggunakan `serializers.serialize("json", ...)` agar berubah jadi string JSON standar yang bisa dibaca sistem lain di luar Python. hasil JSON tersebut lalu dibungkus dalam **HttpResponse** dan dikirim sebagai respons. saat data itu perlu ditampilkan kembali di halaman web, prosesnya dibalik lewat **deserialization** (`serializers.deserialize`) untuk mengubah string JSON tadi kembali menjadi objek Python yang bisa dipakai langsung di template.
